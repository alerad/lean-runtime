...
def _extract_layer(data: bytes, destination: Path) -> None:
    total = 0
    count = 0
...
class EnvironmentBundles:
    ...
    def export(self, environment_id: str, output: Path) -> BundleInfo:
        ...
        root_layer = _tar_gzip(workspace, excluded=packages_dir)
        layers.append(
            _blob_descriptor(
                root_layer,
                LAYER_MEDIA_TYPE,
        ...
        blobs[layers[-1]["digest"]] = root_layer
        ...
        config_descriptor = _blob_descriptor(config, CONFIG_MEDIA_TYPE)
        ...
        manifest_descriptor = _blob_descriptor(
            manifest,
            MANIFEST_MEDIA_TYPE,
            platform={
                ...
                "architecture": {
                    "x86_64": "amd64",
                    "arm64": "arm64",
                }.get(platform_compatibility()["machine"], platform_compatibility()["machine"]),
            },
        )
        blobs[manifest_descriptor["digest"]] = manifest
        index = canonical_json_bytes(
            {"schemaVersion": 2, "mediaType": INDEX_MEDIA_TYPE, "manifests": [manifest_descriptor]}
        )
        entries = {"oci-layout": b'{"imageLayoutVersion":"1.0.0"}', "index.json": index}
        for digest, data in blobs.items():
            entries["blobs/sha256/" + digest.removeprefix("sha256:")] = data
        payload = _oci_archive(entries)
    ...
    def import_bundle(
        ...
        if entries.get("oci-layout") != b'{"imageLayoutVersion":"1.0.0"}':
            raise EnvironmentError("unsupported OCI image layout")
        index = _json_object(entries.get("index.json", b""), "index")
        ...
        manifest_data = _descriptor_blob(entries, manifest_descriptor, "manifest")
        ...
        config = _json_object(_descriptor_blob(entries, config_descriptor, "config"), "config")
        ...
        with FileLock(self.store.lock_dir / f"{environment_id}.lock", timeout=1800):
            if not destination.is_dir():
                ...
                try:
                    ...
                    _extract_layer(
                        _descriptor_blob(entries, root_descriptor, "root layer"), workspace
                    )
                    _verify_workspace_lock(workspace, lock)
                    packages_dir = workspace.joinpath(*_packages_directory(lock).parts)
                    for package in lock.packages:
                        ...
                        package_root = packages_dir / package.name
                        _extract_layer(
                            _descriptor_blob(entries, descriptor, f"package {package.name}"),
                            package_root,
                        )
                        _verify_package(package_root, package)
                    if probe:
        ...
        self.events.emit(
            "bundle.imported", "Imported verified environment bundle", path=str(bundle)
        )
        return BundleInfo(
            environment_id, lock.lock_id, str(manifest_descriptor["digest"]), str(bundle)
        )
