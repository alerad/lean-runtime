...
proofs = [f"import Mathlib\nexample : {i} + 0 = {i} := by norm_num" for i in range(2)] + [
...
assert ok == 2, f"expected 2/3 accepted, got {ok}/3"
bad = results[-1]
...
print(f"python batch API ok: {ok}/3 accepted in {time.time() - started:.1f}s")
